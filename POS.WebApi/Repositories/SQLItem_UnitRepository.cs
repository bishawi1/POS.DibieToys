...
namespace POS.WebApi.Repositories
{
    public class SQLItem_UnitRepository : IItem_UnitRepository
    {
        ...
        {
            Item_UnitModel itemUnit = await dbContext.Item_Unit.FirstOrDefaultAsync(r => r.Item_Unit_ID == itemUnitID);
            return itemUnit;
        }
        ...
        {
            var model = await dbContext.Item_Unit.FirstOrDefaultAsync(r => r.Item_Unit_ID == itemUnitID);
            if (model == null)
            {
                return null;
            }
